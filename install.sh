...
main() {
    ...
    *) _bin_name="abbr" ;;
    ...
    # Download and extract abbr.
    local _package
    _package="$(ensure download_abbr "${_arch}")"
    ...
    *)
    ...
    echo "Installed abbr to ${BIN_DIR}"
    ...
    echo ""
    echo "abbr is installed!"
    if ! echo ":${PATH}:" | grep -Fq ":${BIN_DIR}:"; then
        echo "Note: ${BIN_DIR} is not on your \$PATH. abbr will not work unless it is added to \$PATH."
    fi
}
...
usage() {
    ...
    echo "\
${_text_heading}abbr installer${_text_reset}
imnotzrus <tu0qnqy3n@gmail.com>
https://github.com/imnotzrus/abbr

Fetches and installs abbr. If abbr is already installed, it will be updated to the latest version.
...
download_abbr() {
    ...
    local _releases_url="https://api.github.com/repos/imnotzrus/abbr/releases/latest"
    local _releases
    case "${_dld}" in
    ...
    *) err "unsupported downloader: ${_dld}" ;;
    ...
    (echo "${_releases}" | grep -q 'API rate limit exceeded') &&
        err "you have exceeded GitHub's API rate limit. Please try again later, or use a different installation method: https://github.com/imnotzrus/abbr/#installation"

    local _package_url
    _package_url="$(echo "${_releases}" | grep "browser_download_url" | cut -d '"' -f 4 | grep -- "${_arch}")" ||
        err "abbr has not yet been packaged for your architecture (${_arch}), please file an issue: https://github.com/imnotzrus/abbr/issues"
